<script>
	...
	// The dock mirrors whichever surface is showing; on Videos its tiles must use
	// the video-thumbnail endpoint (cache-busted, so a freshly seeded/re-picked
	// still refreshes) — /api/thumb 404s until a video's still exists.
	function dockThumb(fbid) {
		return showVideos ? videoTileSrc(fbid) : thumbUrl(fbid);
	}
	...
	<section class="flex min-w-0 flex-1 flex-col lg:min-h-0 mr-5">
		...
		{:else if showVideos}
			<header class="mb-2 shrink-0 pt-1 pb-1">
				...
				<p class="mt-2 text-sm text-surface-600">
					Videos are kept as thumbnails. Double-click a video to play it and
					choose the frame, or right-click for “Choose Thumbnail”. Enable selection to pick
					videos with a click, just like albums. They are not auto-kept.
					<br>
		...
		<div class="drag-ghost-layer pointer-events-none fixed inset-0">
			<div class="ghost-stack absolute" style="left: {ghostPos.x}px; top: {ghostPos.y}px;">
				{#each ghostCards as card, i (card.key)}
					...
					>
						{#if card.photo}
							<img src={dockThumb(card.photo.fbid)} alt="" draggable="false" />
						{:else}
